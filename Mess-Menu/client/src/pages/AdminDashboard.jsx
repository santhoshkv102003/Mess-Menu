...
const AdminDashboard = () => {
    ...
    const fetchAnalytics = async () => {
        try {
            ...
            const res = await api.get(`/admin/analytics?month=${month}`);
                                        ...
                                        <table className="w-full text-left text-sm">
                                            <thead className="bg-slate-50 dark:bg-slate-900/50 text-slate-500 uppercase text-xs tracking-wider font-bold">
                                                <tr>
                                                    ...
                                                    <th className="p-4 text-right">Vote Count</th>
                                                </tr>
                                            </thead>
                                            <tbody className="divide-y divide-slate-200 dark:divide-slate-700 text-slate-700 dark:text-slate-300">
                                                {analyticsData.map((item) => (
                                                    <tr key={item._id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                                                        ...
                                                        <td className="p-4">
                                                            <span className="px-2 py-1 rounded-lg bg-slate-100 dark:bg-slate-800 text-xs font-bold uppercase tracking-wide text-slate-500">
                                                                {item.category}
                                                            </span>
                                                        </td>
                                                        <td className="p-4">
                                                            <span className={`px-2 py-1 rounded-lg text-xs font-bold uppercase tracking-wide ${item.dietType === 'Non-Veg'
                                                        ...
                                                        <td className="p-4 text-right font-bold text-lg">{item.voteCount}</td>
                                                    </tr>
                                                ))}
                                            </tbody>
                                        </table>
